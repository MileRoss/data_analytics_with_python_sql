...
-- "Fully qualified column" consist of database_name.table_name.column_name
...
SELECT DISTINCT bank.card.type
FROM bank.card;
...
SELECT bank.district.A2, bank.district.A3
FROM bank.district
ORDER BY bank.district.A2
LIMIT 30;
...
SELECT 
bank.district.A2 AS district_name, 
bank.district.A11 AS average_salary
FROM bank.district
WHERE bank.district.A11 > 10000;
...
SELECT 
bank.loan.amount - bank.loan.payments AS debt_value, 
bank.loan.loan_id, 
bank.loan.account_id 
FROM bank.loan
WHERE bank.loan.status = 'B';
...
SELECT 
bank.district.A2 AS district_name,
bank.district.A4 * bank.district.A10 / 100 AS urban_population
FROM bank.district;
...
SELECT *
FROM bank.order
WHERE NOT (k_symbol = 'SIPO' OR amount < 1000);
...
-- Function to retrieve a given number of characters of a string starting from the left
...
SELECT A2, RIGHT(A2,5) FROM bank.district;

-- Functions to drop white spaces of a string
SELECT LTRIM(' Hello ');
SELECT RTRIM(' Hello ');
...
SELECT * FROM bank.card
WHERE bank.card.type = 'junior' AND LEFT(bank.card.issued,2) >= 98;
...
SELECT * FROM bank.trans
WHERE bank.trans.type = 'VYDAJ' AND NOT bank.trans.operation = 'VYBER'
LIMIT 10;
...
FROM bank.loan
WHERE bank.loan.status = 'B' 
AND (bank.loan.amount - bank.loan.payments) > 1000
...
SELECT *, SUBSTR(date,1,2) AS year
FROM bank.account;
...
SELECT 
bank.card.card_id,
DATE_FORMAT(SUBSTRING_INDEX(bank.card.issued, ' ', 1), '%Y') AS 'year_issued'
FROM bank.card
WHERE bank.card.type = 'gold';
...
FROM bank.card
WHERE bank.card.type = 'gold'
ORDER BY bank.card.issued ASC
LIMIT 1;

-- 3. Get issue date as:
--    - date_issued: 'November 7th, 1993'( American format)
SELECT
DATE_FORMAT(SUBSTRING_INDEX(bank.card.issued, ' ', 1), '%M %D %Y') as date_issued
FROM bank.card;

--    - fecha_emision: '07 of November of 1993' (Spanish format)
SELECT
DATE_FORMAT(SUBSTRING_INDEX(bank.card.issued, ' ', 1), '%d of %M of %Y') as fecha_emision
FROM bank.card;
...
SELECT ISNULL(bank.loan.amount) FROM bank.loan;
...
SELECT 
SUM(CASE WHEN bank.order.k_symbol IS NULL THEN 1 END) AS empty_values,
SUM(CASE WHEN bank.order.k_symbol IS NOT NULL THEN 1 END) AS non_empty
FROM bank.order;
-- ==================================================
...
-- We need to be very careful when dealing with dates. In our case, the `date` values of table trans are integers! Then we can use a simple condition to refine our query but base on integers.
...
SELECT * FROM bank.trans
WHERE bank.trans.date BETWEEN 930101 AND 930115;

-- 2. Find the different values from the field `A2` that start with the letter 'M'.
SELECT * FROM bank.district
WHERE bank.district.A2 LIKE 'M%';
...
SELECT
SUM(bank.trans.amount) AS total_revenue
FROM bank.trans;

-- 2. Get the total rental revenue by customer_id sorted by total benefit in descending order.
SELECT
bank.order.account_to,
SUM(bank.order.amount) AS total_revenue
FROM bank.order
GROUP BY bank.order.account_to
